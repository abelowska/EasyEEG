...
def filter(epochs_data, subject=slice(None), condition=slice(None), trial=slice(None), channel=slice(None),
           timepoint=slice(None), filter_dict=dict()):
...
def generate_case_data(case_frame, epochs_data_todo):
    ...
    for target_level in levels[:-1]:
        ...
        extracted_data[target_level + '_group'] = [i.split(' ')[0] + ' ' + case_frame_dict[target_level + '_name'] for i
                                                   in extracted_data.index.get_level_values(target_level)]
