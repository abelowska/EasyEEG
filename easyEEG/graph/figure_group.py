...
def plot(self, plot_params=None, save=False, return_fig=False):
    ...
    fig = plt.figure(figsize=(fig_x, fig_y), dpi=200)
...
def select_subplot_type(subplot_type, ax, data, analysis_name, annotation, plot_params):
    if subplot_type == 'waveform':
        ...
        figure_unit.plot_heatmap(data, analysis_name, plot_params, ax)
        if 'compare' in plot_params and plot_params['compare'] == True:
...
def matrix_plot(data, x_axis, y_axis, plot_params, annotation=None, fig=None):
    def pre(target):
        ...
        for level_name in target.index.names:
            if '_group' in level_name:
                ...
                target.index = target.index.set_levels([' '.join(i.split(' ')[1:]) for i in old_values_in_level],
                                                       level=level_name)
    ...
    x_axis_values = list(OrderedDict.fromkeys(
        data.index.get_level_values(x_axis)))  # remove duplicates from a list in whilst preserving order
    y_axis_values = list(OrderedDict.fromkeys(
        data.index.get_level_values(y_axis)))  # remove duplicates from a list in whilst preserving order
    ...
    inner_grids = gridspec.GridSpec(4, 3, wspace=wspace, hspace=hspace, height_ratios=height_ratios,
    ...
    cb1 = matplotlib.colorbar.ColorbarBase(ax, cmap=plot_params['color'],
                                           norm=matplotlib.colors.Normalize(vmin=plot_params['zlim'][0],
                                                                            vmax=plot_params['zlim'][1]),
