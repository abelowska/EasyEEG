...
def refine_axis(title, xticklabels, plot_params, ax):
    ...
    if len(xticks) >= 40:
        ...
        xticks_fine = [(label_fine - xticklabels_min) / (label - xticklabels_min) * tick for tick, label, label_fine in
                       zip(xticks, xticklabels, xticklabels_fine)]
...
def plot_heatmap(data, plot_params={'grid': True, 'color': sns.cubehelix_palette(light=1, as_cmap=True),  'vmin':-1, 'vmax':2}, ax=None):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
    ...
    if 'cbar_values' in plot_params:
        ...
        sns.heatmap(data, ax=ax, cbar_ax=cbar_ax, cmap=cmap,  vmin=plot_params['vmin'], vmax=plot_params['vmax'])
...
def plot_topograph(amp_data, pvalue_data, plot_params={'color': plt.cm.jet}, ax=None):
    ...
    corners = pd.DataFrame([[0, left, bottom], [0, right, bottom], [0, left, top], [0, right, top]],
                           columns=['Amp', 'x_locs', 'y_locs'])
    ...
    if 'mask' not in plot_params or plot_params['mask']:
        ...
        ax.add_patch(circle)
        # draw a nose
        points = [(xy_center[0] - (radius - 0.5) / 5, xy_center[1] + radius - 0.5),
                  (xy_center[0], xy_center[1] + radius),
                  (xy_center[0] + (radius - 0.5) / 5, xy_center[1] + radius - 0.5)]
    ...
    if isinstance(pvalue_data, pd.Series):
        ...
        for ind, i in pvalue_data.iterrows():
            if i.pval < plot_params['sig_limit']:
                ax.scatter(i.y_locs - bottom, i.x_locs - left, c='w', s=25, alpha=1, zorder=3, linewidths=0.5,
                           edgecolors='b')
